...
private fun BankApp(vm: BankViewModel, activity: FragmentActivity) {
    ...
    // KYC gate — skip for admin accounts
    val isAdmin = state.user?.role == "admin"
    val kycStatus = state.user?.kycStatus ?: "none"
    if (!isAdmin && kycStatus != "approved") {
        KycScreen(state = state, vm = vm)
        return
    }

    // Main App Shell once logged in, unlocked and KYC verified
    MainShell(state, vm)
}
...
private fun MainShell(state: BankUiState, vm: BankViewModel) {
    ...
    // Settings overlay (full screen, no bottom nav)
    if (showSettings) {
        SettingsScreen(
            state = state,
            vm = vm,
